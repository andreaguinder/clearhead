...
export default function TaskChecklist({ items = [], onUpdateChecklist }: TaskChecklistProps) {
  ...
  // 1. Alternar el estado completado (Tildar / Destildar)
  ...
  // 2. Agregar un nuevo ítem a la lista
  ...
  // 3. Eliminar un ítem (Usando el tacho de Lucide)
  const deleteItem = (id: string) => {
    const updated = items.filter(item => item.id !== id);
    onUpdateChecklist(updated);
  };

  // 4. Guardar edición al salir del input (Blur) o presionar Enter
